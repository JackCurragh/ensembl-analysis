...
sub default_options {
    ...
    return {
        ...
        cdna_fasta      => undef,   # cDNA / mRNA FASTA (optional)
        protein_fasta   => undef,   # protein FASTA — UniProt/RefSeq (optional)
    };
}
...
sub pipeline_analyses {
    ...
    $nf_params{cdna_fasta}    = $self->o('cdna_fasta')    if defined $self->o('cdna_fasta');
    $nf_params{protein_fasta} = $self->o('protein_fasta') if defined $self->o('protein_fasta');
    ...
    return [
...

        {
            ...
            -parameters  => { cmd => 'echo "BestTargeted output ready: type=#type# path=#path#"' },
            -meadow_type => 'LOCAL',
        },
