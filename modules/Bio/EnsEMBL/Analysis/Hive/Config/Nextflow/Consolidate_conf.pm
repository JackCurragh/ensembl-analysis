...
Usage:
  ...
  init_pipeline.pl Bio::EnsEMBL::Analysis::Hive::Config::Nextflow::Consolidate_conf \
    -pipeline_db    "-host localhost -port 3306 -user ensrw -pass XXX -dbname consolidate_pipe" \
    -gff3_dir       /data/layers \
    -outdir         /data/output/consolidated \
    -nextflow_work_root /data/nf_work \
    -nf_pipeline_dir    /path/to/ensembl-genes-nf/pipelines/consolidate
...
sub default_options {
    ...
    return {
        ...
        # ----------------------------------------------------------------
        # Required inputs — provide gff3_dir OR gff3_files
        # ----------------------------------------------------------------
...
sub pipeline_analyses {
    ...
    if (defined $self->o('gff3_dir')) {
        $nf_params{gff3_dir} = $self->o('gff3_dir');
    }
    if (defined $self->o('gff3_files')) {
        $nf_params{gff3_files} = $self->o('gff3_files');
    }
    ...
    return [
...

        {
            ...
            -parameters  => {
                cmd => 'echo "Consolidate output ready: type=#type# path=#path#"',
            },
            -meadow_type => 'LOCAL',
        },
